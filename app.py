...
def get_order_symbol(symbol: str) -> str:
    """Returns the standard hyphenated contract identifier (BTC-USDT)."""
...
def get_symbol_variants(symbol: str) -> list[str]:
    """Generates both hyphenated and unhyphenated symbol representations."""
    clean = symbol.replace("-", "").replace("_", "").replace(".P", "").replace("/", "").upper()
...
def delete_single_order(client_order_id: str) -> bool:
    """
    Conditions 6 & 7: Cancels order via exact JSON schema.
...
def emergency_market_close(symbol: str, side: str, quantity: float):
    ...
    clean_target = symbol.replace("-", "").replace("_", "").replace(".P", "").replace("/", "").upper()
    order_symbol = get_order_symbol(symbol)
    symbols_to_try = [order_symbol, clean_target]

    for target_sym in symbols_to_try:
        try:
            ts = str(int(time.time() * 1000))
            close_params = {
                "deviceType": "WEB",
            ...
            print(f">>> [MARKET FLATTEN ({target_sym}) RESULT HTTP {resp.status_code}]: {resp.text.strip()}")
...
def place_stop_loss(symbol: str, side: str, quantity: float, stop_price: float, ref_price: float = 0.0) -> str:
    """Submits a STOP_LIMIT order with a 15 pt limit buffer using the verified hyphenated ticker."""
    ...
    try:
        ...
        for target_sym in symbols_to_try:
            ...
            print(f"[SL SUBMIT] Placing {side} STOP_LIMIT ({target_sym}) @ Stop: {clean_stop_price}, Limit: {clean_limit_price}...")
            resp = requests.post(
                f"{SHARK_BASE_URL}/v1/order/place-order",
            ...
            if resp.status_code in [200, 201]:
                ...
                print(f"[SL ATTEMPT {target_sym} HTTP {resp.status_code}]: {resp.text}")
...
def execute_entry_order(action: str, symbol: str, quantity: float, target_limit_price: float, trade_id: int):
    """Handles reversals, liquidations, and entry limit placement with hyphenated ticker fallback."""
    ...
    with ORDER_EXECUTION_LOCK:
        try:
            ...
            symbols_to_try = [order_symbol, clean_symbol]
            ...
            for target_sym in symbols_to_try:
                ...
                print(f"\n[LIMIT ENTRY] Placing {side} {clean_qty} ({target_sym}) @ Limit Price {clean_price}...")
                ...
                if resp_entry.status_code in [200, 201]:
                    ...
                    print(f"[LIMIT ENTRY ATTEMPT {target_sym} HTTP {resp_entry.status_code}]: {resp_entry.text}")
